...
def plot_res(envs, algos, metric, smoothing, ylabel, xlabel):
    fig, axes = plt.subplots(nrows=1, ncols=len(envs), figsize=(4.0 * len(envs), 4.0))
    ...
    for ax, env in enumerate(envs):
        ...
        for i, v_each in enumerate(v_all):
            if isinstance(v_each, np.ndarray):
                axes[ax].plot(
                    smooth(np.percentile(v_each, q=50, axis=0), smoothing),
                    label=algos[i],
                )
                axes[ax].fill_between(
                    x=np.arange(v_each.shape[1] - smoothing + 1),
                    y1=smooth(np.percentile(v_each, q=10, axis=0), smoothing),
                    y2=smooth(np.percentile(v_each, q=90, axis=0), smoothing),
                    alpha=0.4,
                )
