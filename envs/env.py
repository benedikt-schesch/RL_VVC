...
class VVCEnv:
    def __init__(self,
        ...
        if self.state_option in ('1', '3' , '4' , '5'):
    ...
    def reset(self, offline=True):
        ...
        elif self.state_option == '4':
            self.state = np.concatenate([
                np.zeros_like(self.substation_pq[self.global_time-1, :] / (self.basekVA / 3.)),
                np.zeros_like(self.load[self.global_time, :] / self.load_avg),
            ])
        elif self.state_option == '5':
            self.state = np.concatenate([
                self.substation_pq[self.global_time-1, :] / (self.basekVA / 3.),
                self.load[self.global_time, :] / self.load_avg,
            ])
    ...
    def step(self, action=None):
        ...
        # reward
        if info['PF converge']:
            if self.reward_option in ('1', '2', '5'):
        ...
        if self.state_option == '1':
            self.state = np.concatenate([
                ...
                np.array(load_kvar+load_kw) / self.load_avg if self.env_name == '8500' else np.array(load_kw+load_kw) / self.load_avg,
            ])
        elif self.state_option == '2':
            self.state = np.concatenate([
                substation_pq / (self.basekVA / 3.),
            ...
            self.state = np.concatenate([
                np.zeros_like(substation_pq / (self.basekVA / 3.)),
                np.zeros_like(self.load[self.global_time, :] / self.load_avg),
            ])
        elif self.state_option == '5':
            self.state = np.concatenate([
